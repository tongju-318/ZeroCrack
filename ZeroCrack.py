...
class ZeroCrack(nn.Module):
    def __init__(self, checkpoint_path=None, img_size=336) -> None:
        super(ZeroCrack, self).__init__()
        self.zerocrack_vit = _create_vit_backbone(img_size)
        if checkpoint_path:
            ...
            self.zerocrack_vit.load_state_dict(new_ckpt, strict=False)
        for param in self.zerocrack_vit.parameters():
            param.requires_grad = False
        blocks = []
        for block in self.zerocrack_vit.blocks:
            blocks.append(
                Adapter(block)
            )
        self.zerocrack_vit.blocks = nn.Sequential(
            *blocks
        )
