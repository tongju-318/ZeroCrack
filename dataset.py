...
class TestDataset(Dataset):
    """
    专门用于推理的 Dataset，支持返回原始尺寸和文件名
    """

    def __init__(self, image_root, size, gt_root=None):
        self.size = size
        self.images = sorted([os.path.join(image_root, f) for f in os.listdir(image_root)
                              if f.lower().endswith(('.jpg', '.png', '.jpeg'))])
    ...
    def __getitem__(self, idx):
        ...
        # 将 w 和 h 作为独立的 int 返回，DataLoader 会把它们打包成 [Batch] 形状的 Tensor
        return image, name, w, h
